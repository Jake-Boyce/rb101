...
def print_in_box(str='')
  ...
  if str.length > 76
    char_index = 75
    is_space = str[char_index] == ' '
    while is_space == false do
      char_index -= 1
      is_space = str[char_index] == ' '
    end
    next_line = str.slice!(char_index..str.length)
  end
  ...
  unless next_line.empty?
    next_length = next_line.length
    next_line.prepend('| ')
    next_line << "#{' ' * (str.length - next_length)}"
    next_line << ' |'
  end
  ...
  puts empty_line
  puts str_line
  puts next_line unless next_line.empty?
  puts empty_line
  puts top_and_bottom
end
